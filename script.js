...
function render(){
 ...
 const hp=$('#home .hero-copy>p');
 if(hp) hp.textContent=c.heroText||'Reliable contract manpower and cargo transport solutions built for industrial and corporate environments.';
 ...
 const divGrid=document.querySelector('.division-grid');
 if(divGrid&&Array.isArray(C.divisions)) divGrid.innerHTML=C.divisions.map((d,i)=>`<article><span>${String(i+1).padStart(2,'0')}</span><h3>${safe(d.name||d.title)}</h3><p>${safe(d.description||d.text)}</p></article>`).join('');
 ...
 if(why&&Array.isArray(C.whyUs)) why.innerHTML=C.whyUs.map((x,i)=>`<div><b>${String(i+1).padStart(2,'0')}</b><h3>${safe(x)}</h3><p>Operational capability supporting dependable project execution.</p></div>`).join('');
...
function setupTabs(){
 document.querySelectorAll('#nav a[href^="#"], a.btn[href^="#"]').forEach(a=>a.addEventListener('click',e=>{
...
async function load(){
 try{
   const r=await fetch('/content.json?version='+Date.now(),{cache:'no-store'});
   if(r.ok){const fresh=await r.json(); if(fresh&&typeof fresh==='object') C=fresh;}
